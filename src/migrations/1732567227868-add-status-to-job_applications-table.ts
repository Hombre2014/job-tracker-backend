...
export class AddStatusToJobApplicationsTable1732567227868 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."job_applications_status_enum" AS ENUM('Job Created', 'Deadline', 'Applied', 'Interview', 'Offer Received', 'Job Moved')`,
    );
    await queryRunner.query(
      `ALTER TABLE "job_applications" ADD "status" "public"."job_applications_status_enum" NOT NULL DEFAULT 'Job Created'`,
    );
  }
